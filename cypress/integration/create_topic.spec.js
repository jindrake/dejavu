...
describe('sign-up then create topic', () => {
  beforeEach(() => {
    indexedDB.deleteDatabase('firebaseLocalStorageDb')
    cy.visit(`http://localhost:3000/sign-up`)
  })

  it.only('signup a new user and gets redirected to home to create a new topic', () => {
    const firstName = faker.name.firstName()
    const lastName = faker.name.lastName()
    const email = faker.random.alphaNumeric(10) + faker.internet.email()
